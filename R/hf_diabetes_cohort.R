#!/usr/bin/Rscript
#############################################################################
### Patients can be defined via patient_id or patient_sk.  
### patient_sk preferred in theory, but correctness questionable.
#############################################################################
# Confounders:
#   HbA1c increase over time?  Adjust for year by including avg date of labs.
#   Age of patient.
...
yob <- unique(subset(c0, select = c("patient_id","yob")))
yob <- aggregate(x = yob$yob, by = list(yob$patient_id), FUN = min)
colnames(yob) <- c("patient_id","yob")
c0$yob <- NULL
c0 <- merge(c0, yob, all.x=T, all.y=F, by="patient_id")
#
...
#
gender <- unique(subset(c0, select = c("patient_id", "gender")))
gender <- aggregate(x = gender$gender, by = list(gender$patient_id), FUN = unanimous)
colnames(gender) <- c("patient_id","gender")
writeLines(sprintf("Npatients with conflicting gender data: %d", nrow(gender[gender$gender=="CONFLICT",])))
c0$gender <- NULL
c0 <- merge(c0, gender, all.x=T, all.y=F, by="patient_id")
c0 <- c0[c0$gender!="CONFLICT",]
#
race <- unique(subset(c0, select = c("patient_id", "race")))
race <- aggregate(x = race$race, by = list(race$patient_id), FUN = unanimous)
colnames(race) <- c("patient_id","race")
writeLines(sprintf("Npatients with conflicting race data: %d", nrow(race[race$race=="CONFLICT",])))
c0$race <- NULL
c0 <- merge(c0, race, all.x=T, all.y=F, by="patient_id")
...
#
###
cohort <- unique(subset(c0, select = c("patient_id", "yob", "gender", "race")))
cohort <- cohort[order(cohort$patient_id),]
...
#
#Dx per patient:
dx_counts <- aggregate(x = c0$diagnosis_code, by = list(c0$patient_id), FUN = length)
colnames(dx_counts) <- c("patient_id", "n_dx")
cohort <- merge(cohort, dx_counts, all.x=T, all.y=F, by="patient_id")
#
...
#
write.table(cohort$patient_id, file = "data/hf_diabetes_cohort.pid", row.names=F, col.names=F, quote=F)
...
c0 <- merge(cohort, subset(c0, select = c("patient_sk","patient_id","diagnosis_id","diagnosis_code","dx_date","diagnosis_description","age_in_years")), all.x=T, all.y=F, by="patient_id")
c0 <- c0[order(c0$patient_id),]
#
...
diags <- read_csv("data/hf_diabetes_cohort_f_diagnosis.csv.gz", col_types = cols(PID = col_integer(), SK = col_integer(), FID = col_integer(), Date = col_date(format = "%Y-%m-%d")))
###
#
...
###
#Dx by gender:
diags <- merge(diags, cohort, all.x=T, all.y=F, by.x="PID", by.y="patient_id")
n_diags_female <- nrow(diags[diags$gender=="Female",])
n_diags_male <- nrow(diags[diags$gender=="Male",])
writeLines(sprintf("Females in diags: %d",length(unique(diags$PID[diags$gender=="Female"]))))
writeLines(sprintf("Males in diags: %d",length(unique(diags$PID[diags$gender=="Male"]))))
writeLines(sprintf("Total patients in diags: %d",length(unique(diags$PID))))
#
#
...
meds <- read_csv("data/hf_diabetes_cohort_f_medication.csv.gz", col_types = cols(PID = col_integer(), SK = col_integer(), FID = col_integer(), Date = col_date(format = "%Y-%m-%d")))
###
...
writeLines(sprintf("Total patients with glucose-lowering non-insulin meds:
%d",length(unique(glnids$PID))))
d <- data.table(table(glnids$generic_name))
d <- d[order(-d$N),]
writeLines(sprintf("Rx_GLNID = %24s: N_med = %8d",d$V1, d$N))
glnids_by_id <- unique(subset(glnids, select = c("PID", "generic_name")))
d2 <- data.table(table(glnids_by_id$generic_name))
d2 <- d2[order(-d2$N),]
writeLines(sprintf("Rx_GLNID = %24s: N_patient = %8d",d2$V1, d2$N))
pids_glnid <- unique(glnids$PID)
#
###
#Insulins:
###
...
#
pids_insulins <- unique(insulins$PID)
pids_insulins_e <- unique(insulins$PID[insulins$ins=="ENGINEERED"])
pids_insulins_h <- unique(insulins$PID[insulins$ins=="HUMAN"])
pids_insulins_e_only <- setdiff(pids_insulins_e,pids_insulins_h)
pids_insulins_h_only <- setdiff(pids_insulins_h,pids_insulins_e)
writeLines(sprintf("Total patients with insulin meds: %d",length(pids_insulins)))
writeLines(sprintf("Patients with ENGINEERED insulin meds: %d",length(pids_insulins_e)))
writeLines(sprintf("Patients with HUMAN insulin meds: %d",length(pids_insulins_h)))
writeLines(sprintf("Patients with BOTH HUMAN and ENGINEERED insulin meds:
%d",length(intersect(pids_insulins_h,pids_insulins_e))))
writeLines(sprintf("Patients with ONLY ENGINEERED insulin meds: %d",length(pids_insulins_e_only)))
writeLines(sprintf("Patients with ONLY HUMAN insulin meds: %d",length(pids_insulins_h_only)))
#
#
#SWITCHERS: Patients who switch once, H2E or E2H.
#For each patient in meds, find:
#  - 1st & last E-insulin dates
#  - 1st & last H-insulin dates
insulins_e <- subset(meds, medication_id %in% MID_INSULIN_ENG, select = c(PID, Date))
insulins_e_first <- aggregate(x=insulins_e$Date, by=list(insulins_e$PID), FUN=min) 
colnames(insulins_e_first) <- c("patient_id", "date_eins_first")
insulins_e_last <- aggregate(x=insulins_e$Date, by=list(insulins_e$PID), FUN=max) 
colnames(insulins_e_last) <- c("patient_id", "date_eins_last")
#
insulins_h <- subset(meds, medication_id %in% MID_INSULIN_HUM, select=c(PID,Date))
insulins_h_first <- aggregate(x=insulins_h$Date, by=list(insulins_h$PID), FUN=min) 
colnames(insulins_h_first) <- c("patient_id", "date_hins_first")
insulins_h_last <- aggregate(x=insulins_h$Date, by=list(insulins_h$PID), FUN=max) 
colnames(insulins_h_last) <- c("patient_id", "date_hins_last")
#
insulins_eh <- merge(insulins_h_first, insulins_h_last, all=T, by="patient_id")
insulins_eh <- merge(insulins_eh, insulins_e_first, all=T, by="patient_id")
insulins_eh <- merge(insulins_eh, insulins_e_last, all=T, by="patient_id")
#
insulins_h2e <- insulins_eh[!is.na(insulins_eh$date_hins_last) & !is.na(insulins_eh$date_eins_first) & insulins_eh$date_hins_last<insulins_eh$date_eins_first, ]
pids_insulins_h2e <- unique(insulins_h2e$patient_id)
writeLines(sprintf("Patients H-to-E insulin switchers: %d",length(pids_insulins_h2e)))
insulins_e2h <- insulins_eh[!is.na(insulins_eh$date_eins_last) & !is.na(insulins_eh$date_hins_first) & insulins_eh$date_eins_last<insulins_eh$date_hins_first, ]
pids_insulins_e2h <- unique(insulins_e2h$patient_id)
writeLines(sprintf("Patients E-to-H insulin switchers: %d",length(pids_insulins_e2h)))
#
insulins_h2e$switch <- "H2E"
insulins_h2e$date_switch <- insulins_h2e$date_eins_first
insulins_e2h$switch <- "E2H"
insulins_e2h$date_switch <- insulins_e2h$date_hins_first
insulins_switchers <- rbind(insulins_h2e, insulins_e2h)
pids_insulins_switchers <- unique(insulins_switchers$patient_id)
#
...
events <- read_csv("data/hf_diabetes_cohort_f_clinical_event.csv.gz", col_types = cols(PID = col_integer(), SK = col_integer(), FID = col_integer(), Date = col_date(format = "%Y-%m-%d")))
###
...
events_bmi <- merge(events_bmi, cohort, all.x=T, all.y=F, by.x="PID", by.y="patient_id")
...
events_wt <- merge(events_wt, cohort, all.x=T, all.y=F, by.x="PID", by.y="patient_id")
...
labs <- read_csv("data/hf_diabetes_cohort_f_lab.csv.gz", col_types = cols(PID = col_integer(), SK = col_integer(), FID = col_integer(), Date = col_date(format = "%Y-%m-%d")))
###
...
pids_hba1c <- unique(labs_hba1c$PID)
writeLines(sprintf("Total HbA1c labs: %d",nrow(labs_hba1c)))
writeLines(sprintf("Patients with HbA1c labs: %d",length(pids_hba1c)))
labs_hba1c <- labs_hba1c[labs_hba1c$Result>=HBA1C_MIN & labs_hba1c$Result<=HBA1C_MAX,]
pids_hba1c <- unique(labs_hba1c$PID)
writeLines(sprintf("Sanity filtering HbA1c labs, range: [%.1f, %.1f]",HBA1C_MIN,HBA1C_MAX))
writeLines(sprintf("Total HbA1c labs (SANE): %d",nrow(labs_hba1c)))
writeLines(sprintf("Patients with HbA1c labs (SANE): %d",length(pids_hba1c)))
#
writeLines("===== FILTER COHORT FOR SUFFICIENT DATA:")
#
pids_ins_hba1c <- intersect(pids_insulins,pids_hba1c)
writeLines(sprintf("Patients with insulin meds AND HbA1c labs: %d",length(pids_ins_hba1c)))
#
pids_ins_h_only_hba1c <- intersect(pids_insulins_h_only, pids_hba1c)
pids_ins_e_only_hba1c <- intersect(pids_insulins_e_only, pids_hba1c)
writeLines(sprintf("Patients with ONLY HUMAN insulin meds AND HbA1c labs: %d",length(pids_ins_h_only_hba1c)))
writeLines(sprintf("Patients with ONLY ENGINEERED insulin meds AND HbA1c labs: %d",length(pids_ins_e_only_hba1c)))
#
cohort <- cohort[cohort$patient_id %in% union(pids_ins_h_only_hba1c,pids_ins_e_only_hba1c),]
#
###
#How many HbA1c's per patient?
hba1c_counts <- aggregate(x = labs_hba1c$Result, by = list(labs_hba1c$PID), FUN = length)
colnames(hba1c_counts) <- c("patient_id", "n_hba1c")
cohort <- merge(cohort, hba1c_counts, all.x=T, all.y=F, by="patient_id")
#
hba1c_mean <- aggregate(x = labs_hba1c$Result, by = list(labs_hba1c$PID), FUN = mean)
colnames(hba1c_mean) <- c("patient_id", "mean_hba1c")
cohort <- merge(cohort, hba1c_mean, all.x=T, all.y=F, by="patient_id")
#
hba1c_meandate <- aggregate(x=labs_hba1c$Date, by=list(labs_hba1c$PID), FUN=mean)
colnames(hba1c_meandate) <- c("patient_id", "hba1c_meandate")
cohort <- merge(cohort, hba1c_meandate, all.x=T, all.y=F, by="patient_id")
#
writeLines(sprintf("Patients with glucose-lowering non-insulin meds: N = %d",nrow(cohort[cohort$patient_id %in% pids_glnid,])))
cohort <- cohort[!(cohort$patient_id %in% pids_glnid),]
#
...
#
cohort$ins <- NA
cohort$ins[cohort$patient_id %in% pids_insulins_e_only] <- "ENGINEERED"
cohort$ins[cohort$patient_id %in% pids_insulins_h_only] <- "HUMAN"
#
setDT(labs_hba1c, key="PID")
...
hgbval_e <- labs_hba1c$Result[labs_hba1c$ins=="ENGINEERED"]
hgbval_h <- labs_hba1c$Result[labs_hba1c$ins=="HUMAN"]
#
tt <- t.test(hgbval_e, hgbval_h , var.equal=F)
...
#
boxplot(hgbval_e, hgbval_h, ylim=c(0,25), names=c("engineered","human"), col="tomato",
        range=1.5, varwidth=T, boxwex=0.5)
title(main="HbA1c vs. Insulin class")
abline(h=mean(hgbval_e), col="gray", lwd=2)
abline(h=mean(hgbval_h), col="gray", lwd=2)
text(1,mean(hgbval_e),sprintf("mean = %.2f",mean(hgbval_e)), pos=3, cex=0.8)
text(2,mean(hgbval_h),sprintf("mean = %.2f",mean(hgbval_h)), pos=1, cex=0.8)
###
vioplot(hgbval_e, hgbval_h, ylim=c(0,25), names=c("engineered","human"), col="tomato", range=1.5, wex=0.5)


title(main="HbA1c vs. Insulin class")
text(1,mean(hgbval_e),sprintf("mean = %.2f",mean(hgbval_e)), pos=4, cex=0.8)
text(2,mean(hgbval_h),sprintf("mean = %.2f",mean(hgbval_h)), pos=4, cex=0.8)
#
#
...
###
#
p2 <- plot_ly(labs_hba1c, y=~Result, color=~ins, type="box",
              boxmean=T, boxpoints = "all", jitter = 0.5, pointpos=0, whiskerwidth=1,
              colors=cols_this, marker=list(symbol="dot", opacity=0.4)) %>%
  add_annotations(x=0, y=mean(labs_hba1c$Result[labs_hba1c$ins=="ENGINEERED"]), xanchor="left",
                  showarrow=F, text=sprintf("%.2f",mean(labs_hba1c$Result[labs_hba1c$ins=="ENGINEERED"]))) %>%
  add_annotations(x=1, y=mean(labs_hba1c$Result[labs_hba1c$ins=="HUMAN"]), xanchor="left",
                  showarrow=F, text=sprintf("%.2f",mean(labs_hba1c$Result[labs_hba1c$ins=="HUMAN"]))) %>%
  layout(title=sprintf("%s: HbA1c values<br>N_total = %d", TITLE_PREFIX, nrow(labs_hba1c)),
         yaxis=list(title="HbA1c", range=c(0,HBA1C_MAX)), xaxis=list(title=""), 
         margin=list(t=100,l=80,b=100,r=80), showlegend=F,
         font=list(family="Arial", size=14))
p2
#
p2.1 <- plot_ly(labs_hba1c, y=~Result, x=~race, color=~ins, type="box",
...
#
###
#Merge switchers with hba1c labs.  Find sub-cohort with before and after labs.
labs_hba1c_first <- aggregate(x=labs_hba1c$Date, by=list(labs_hba1c$PID), FUN=min) 
colnames(labs_hba1c_first) <- c("patient_id", "date_hba1c_first")
labs_hba1c_last <- aggregate(x=labs_hba1c$Date, by=list(labs_hba1c$PID), FUN=max) 
colnames(labs_hba1c_last) <- c("patient_id", "date_hba1c_last")
insulins_switchers_with_labs <- merge(insulins_switchers, labs_hba1c_first, all.x=F, all.y=F, by="patient_id")
insulins_switchers_with_labs <- merge(insulins_switchers_with_labs, labs_hba1c_last, all.x=F, all.y=F, by="patient_id")
writeLines(sprintf("Switchers with HbA1c labs: %d", nrow(insulins_switchers_with_labs)))
###APPARENTLY NONE!
###
