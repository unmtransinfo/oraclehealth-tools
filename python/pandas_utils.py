...
if __name__=='__main__':
  ...
  #print("DEBUG: compression=%s"%compression, file=sys.stderr)

  if re.search('\.csv', args.ifile, re.I): delim=','
  else: delim='\t'

  if args.op == 'csv2tsv':
    df = pandas.read_csv(fin, sep=',', compression=compression)
  ...
  df = pandas.read_csv(fin, sep=delim, compression=compression)
